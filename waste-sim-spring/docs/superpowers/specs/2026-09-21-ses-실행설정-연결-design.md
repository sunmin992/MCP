...
## 11. 실측 — 무엇이 실제로 작업 생성을 막는가

| | q3-obs1 | smoke-v8-20260916-01 | jn-T2-1 |
|---|---:|---:|---:|
| 작업 후보 | 64 | 64 | 23 |
| 붙은 작업 | **0** | **0** | **0** |
| 검토할 칸 | 217 | 212 | 91 |
...
설정 쪽은 잘 나온다 — 필드 42개, 그 중 이름이 다른 것 2개(`numTrucks↔truckCount` ·
`collectionTimeMinutes↔collectionTime`)를 변환 자리로 잡는다. 손으로 쓴 대응표가 필요했던
자리를 코드가 근거와 함께 낸다.
...
24개 파일 안에 있었다.** 경계가 막은 것이 아니다. 추출기가 읽을 수 있었는데 한 번도
인용하지 않았다.

추출된 속성은 `fill` · `peak` · `tripAccs` · `remainingTruckCapacity` 다 — 전부 **엔진 루프의
상태**다. 사용자가 결정할 것(차종 · 대수 · 배차간격)은 선언 표면에 있고, 추출이 거기 닿지
않는다.

**이것이 지금 템플릿 생성을 막는 누락이고, 대조기로는 풀리지 않는다.** 브리핑 5절이 말한
"어떤 누락이 실제 작업 생성을 막는지 측정한다"의 답이 이것이다. 다음 증분은 개체를 더
찾는 것이 아니라 **추출을 결정 표면에 닿게 하는 것**이다.
