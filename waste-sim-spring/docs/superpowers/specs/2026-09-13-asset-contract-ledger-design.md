...
이 설계의 산출물은 그 자리다 — **매개변수 하나마다 값·상태·출처·변환 이력을 쌓는 결정기록**과,
**구조가 바뀌면 다시 계산하는 규칙**, 그리고 **조립 결과를 결정기록과 대조하는 역검증**이다.
...
| 값 해결 — 결정기록·활성 규칙 | `com.wastesim.ledger` | 신규 |
...
어디서 왔는가의 기록이다. 둘 다 한 시점의 사실이다.** 결정기록은 이 둘을 입력으로 받아 매개변수
하나의 결정 이력을 쌓는다. 선언을 결정기록으로 승격하지 않는 이유는, 선언이 세트 해시가 덮는
...
**세트는 그대로 두고, 결정기록이 활성성을 계산한다.** 비활성으로 판정된 항목은 결정기록이
...
허용돼 있다("답을 고치면 다시 수집으로"). 결정기록은 그 전이를 **일으키는** 쪽만 추가한다. 상태
...
## 결정 5 — 역검증은 컴파일 결과를 결정기록과 대조한다

기존 빌더가 만든 `SimulationConfig`를 다시 열어 결정기록과 한 필드씩 맞춘다.

차단 조건:
- 결정기록의 확정값과 설정의 값이 다르다
- 설정에 값이 있는데 결정기록에 결정 레코드가 없다
- 결정은 있는데 `source`가 없다
...
흐름: 호출 시 `purposeField`로 이 값이 어느 매개변수에 쓰일지 먼저 고정한다 → 결과는 결정기록에
...
1. `ParameterDecision` 레코드와 append-only 결정기록 저장소
2. `BasisKind`·`SubtaskAnswerSource` → 결정기록 상태 대응
...
- 골드 수동 구성이 역검증을 통과하지 못한다 → 결정기록이 아니라 계약의 대응표를 고친다
