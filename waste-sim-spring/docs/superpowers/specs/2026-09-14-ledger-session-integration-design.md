...
호출하고, `approveRun()`은 `SubtaskState`만 본다. 결정기록은 그 경로 어디에도 없다.
...
| 컴파일 결과를 결정기록과 대조한다 | 역검증기가 생성·실행 경로에 연결돼 있지 않다 |
...
남는다. 결정기록이 그것들을 대체하는 것이 아니라, 그것들이 보지 않는 것을 본다.

## 결정 1 — 결정기록은 세션이 들고, 함께 저장된다

`JangnyangSubtaskSession`에 `ParameterLedger` 필드를 추가한다.

**왜 별도 저장소를 두지 않는가**: 세션과 결정기록을 각각 저장하면 한쪽만 저장되는 순간이 생기고,
그 순간에 둘은 다른 사실을 말한다. 결정기록 설계가 "현재 값을 따로 저장하지 않고 이력의 마지막으로
...
1. 그 답변을 `AnswerDecisions.fromAnswer(...)`로 결정 레코드로 만들어 결정기록에 붙인다
...
## 결정 3 — 조립부터 막고, 막힌 자리를 다시 묻는다

> **2026-09-14 개정.** 처음 이 절은 단계적 강제였다 — 조립은 경고만 싣고 실행만 막는다.
> 구현하면서 더 나은 길이 드러나 뒤집었다. 아래에 바뀐 결정과 원래 이유를 함께 남긴다.

| 지점 | 결정기록의 역할 |
|---|---|
| `build()` | **차단한다.** `ScenarioLedgerGate`가 검증하고, 막히면 조립이 실패하며 `currentStep()`이 그 질문으로 되돌아간다 |
| `approveRun()` | **차단한다.** ① 결정기록 차단 상태 → 거부 ② 역검증 불일치 → 거부 |

**원래 조립을 막지 않기로 했던 이유**: 결정기록과 기존 checker는 서로 다른 기준을 쓴다. 결정기록은
`BasisKind.NONE`을 미해결로 막지만 checker는 그 필드를 `required=false`로 보고 통과시킬 수
있다. 조립부터 강제하면 지금 통과하던 구성이 갑자기 막히고, 사용자는 막힌 채로 갇히며,
그것이 진짜 결함인지 두 기준의 차이인지 구분할 데이터가 없다.

**왜 뒤집었는가**: 그 우려의 핵심은 "막힌 사용자가 갇힌다"였는데, **되묻는 경로**가 생기면서
사라졌다. 조립이 막히면 `currentStep()`이 막힌 질문을 다시 내고, 사용자가 답하면 조립이
통과한다. 갇히지 않으면 조립을 통과시켜 둘 이유가 없다 — 미해결인 채로 만들어진 명세는
어차피 실행에서 막히고, 그때 알려 주는 것보다 지금 알려 주는 편이 고치기 쉽다.

**검증이 한자리인 것도 이 결정의 일부다.** `ScenarioLedgerGate`는 세션 경로와 MCP 빌더
양쪽에서 같은 검증을 수행한다. 강제 지점이 실행 하나였다면 세션을 거치지 않는 경로는
검증을 건너뛴다.

`BuildStep.ledgerWarnings`는 남는다 — 막지 않는 관찰 항목을 실어 보낼 자리가 여전히 필요하다.
...
**그래서 이 5개는 역검증으로 값을 대조할 수 없다.** `ConfigBackVerifier`는 결정기록의 정규화 값과
...
| 위와 같은 상황에서 조립 | **막히고, `currentStep()`이 그 질문으로 돌아간다** |
| 막힌 질문에 답한 뒤 다시 조립 | 통과한다 — 사용자가 갇히지 않는 것이 결정 3의 전제다 |
| 정상 골드 구성 | 조립도 실행도 막히지 않음 |
| 설정값을 결정기록과 다르게 변조 | 역검증이 실행을 막음 |
...
- 정상 구성이 실행에서 막힌다 → 결정기록 배선이 아니라 `activeWhenByParameter`·`dependents`를 본다
