...
class EdgeTests(unittest.TestCase):
    ...
    def test_aspect_자식에_개체가_와도_된다(self):
        # 정답지: 경로 교통 시스템 ─aspect→ 수거 경로
        self.assertIsNone(shape.edge_error("boundary", "ASPECT", ["stateful", "set"]))

    def test_개체가_aspect_부모가_될_수_있다(self):
        # 정답지: 민원 판정 ─aspect→ 적재초과 판정 · 임대인 점검 판정 · 교통혼잡 판정
        self.assertIsNone(shape.edge_error("stateful", "ASPECT", ["stateful", "stateful"]))

    def test_boundary_가_spec_부모가_될_수_있다(self):
        # 정답지: 시나리오 실험 ─spec→ 단일 실행 …
        self.assertIsNone(shape.edge_error("boundary", "SPEC", ["type"]))

    def test_유형은_부모가_되지_않는다(self):
        self.assertIsNotNone(shape.edge_error("type", "ASPECT", ["stateful"]))
...
class DeclarationTests(unittest.TestCase):
    ...
    def test_spec_은_보지_않는다(self):
        # 직업 축: 열거 상수 선언 한 줄이 5종을 함께 적는다
        me = {"생산직": ev(12), "학생": ev(12)}
        self.assertIsNone(shape.shared_declaration(me, ["생산직", "학생"], "SPEC"))

    def test_aspect_는_본다(self):
        me = {"건물": ev(254), "쓰레기 종류": ev(254)}
        self.assertIsNotNone(shape.shared_declaration(me, ["건물", "쓰레기 종류"], "ASPECT"))
