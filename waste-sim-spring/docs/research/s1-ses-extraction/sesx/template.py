...
설정 필드는 영문이라 건널 다리가 없다. 건너는 것은 코드 기호다. SPEC 은 선택 기호
(`selection.symbol`)가, 속성·분해는 근거가 적은 기호(`evidence.symbol`)가 다리다.
"""
...
def _symbols(element, evidence):
    """이 요소의 근거가 가리키는 코드 기호들. 붙일 때 쓰는 다리다."""
    out = []
...
def _match(symbols, bindings):
    """기호로 붙는 설정 필드 하나. 이름은 보지 않는다."""
    if not symbols:
        return None
    for b in bindings:
...
def _task(point_id, kind, source, element_slots, b, act, selection, evidence_ids):
    ...
    return {"point_id": point_id, "kind": kind, "source": source,
            ...
            "binding_id": b["binding_id"] if b else None,
...
def draft(artifact, bindings=(), selection=None):
    ...
    def add(point_id, kind, source, slots, b, act, evidence_ids):
        if b is not None:
            used.add(b["binding_id"])
        tasks.append(_task(point_id, kind, source, slots, b, act, selection, evidence_ids))
    ...
    for d in artifact.get("decompositions") or []:
        ...
        b = _match([symbol] + _symbols(d, evidence) if symbol else _symbols(d, evidence),
                   bindings)
        add(point, TASK_OF[d["kind"]], {"element": "decomposition", "id": d["id"]},
            slots, b, act, d.get("evidence_ids"))

    for a in artifact.get("attributes") or []:
        owner = _name(entities, a["entity_id"])
        b = _match(_symbols(a, evidence), bindings)
        add(f"attr:{owner}:{a['name']}", ATTRIBUTE_TASK, {"element": "attribute", "id": a["id"]},
            {"decides": {"entity": owner, "attribute": a["name"]}}, b,
    ...
    for c in artifact.get("couplings") or []:
        ...
        add(f"coupling:{src}->{dst}", COUPLING_TASK, {"element": "coupling", "id": c["id"]},
            {"decides": {"from": src, "to": dst}}, _match(_symbols(c, evidence), bindings),
    ...
    for a in artifact.get("activation") or []:
        ...
        add(f"active:{target or a['id']}", ACTIVATION_TASK,
            ...
            {"decides": {"target": target, "state": a.get("state")}}, None, a,
            a.get("evidence_ids"))
