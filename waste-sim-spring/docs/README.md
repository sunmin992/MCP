...
**장량동 항목의 현재 상태**는 통합 명세서 부록 B에 있고, 미해결은 없다. 코드-문서 불일치는 부록 C.2가
번호로 관리한다(현재 13건 — 그중 코드 수정이 필요한 것은 C-04·C-05·C-11·C-12·C-13).

> 위 문단의 `E-xx` 번호(E-01·E-02·E-04·E-06·E-08)와 게이트 어휘 결함 `D-33`은 **엣지 발열 도메인이
> 함께 있던 시절의 항목**이다. 그 도메인은 2026-08-31에 분리됐으므로 해당 번호는 엣지 문서 쪽에서
> 찾아야 한다. 이 저장소의 번호 체계는 통합 명세서 v2.0에서 1번부터 다시 매겨졌다(부록 A.2 대응표).
...
**현행 — 파일 v1.15 / 문서 버전 2.3 (2026-09-03).** 2,296행 Markdown이며 표 100여 개다. 이 판까지
반영된 것은 (1) 엣지 도메인 분리 완료 확인과 MCP 노출 8종(문서 2.2), (2) 이동시간·좌표 계층
신설(1.15·2.17·3.14), (3) 고정 서브태스크 세트 v3 편입(1.6.2·2.2.5·3.13.6)이다. 기준 회귀는
`@Test` 506개다.

**대기 중 — 문서 버전 2.4.** HEAD 커밋 `4005682`(구역 배정 `ZoneAssignmentRule`, 경로 해석
`RoutePlanner` 단일화, 결과 사용 등급)를 편입한 판이 작성돼 있으나 **아직 이 저장소에 커밋되지
않았다.** 그래서 위 기준 명세서 링크는 여전히 v1.15를 가리킨다 — 그 판은 구역 배정과 결과 사용
등급을 모르므로, 그 두 축을 확인할 때는 문서가 아니라 코드와 테스트
(`ZoneAssignmentRuleTest`·`RoutePlannerTest`)를 봐야 한다. 커밋하면 파일을 v1.16으로 추가하고
이 인덱스의 기준 링크를 함께 갱신할 것.

> 아래는 **엣지 발열 도메인이 함께 있던 시절의 검증 메모**다(파일 v1.7~v1.11). 테스트 건수·클래스
> 수·프론트엔드 줄 수는 모두 엣지를 포함한 값이라 현행 기준선이 아니다. 분리 이후의 기준선은
> 통합 명세서 3.10에 있다.
