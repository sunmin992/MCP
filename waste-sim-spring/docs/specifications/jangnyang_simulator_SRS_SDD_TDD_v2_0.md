...
> 판정 등 앞으로 추가할 설계는 아직 이 문서에 없다** — 그 자리는 2.2와 3.1에 빈 자리로 표시해 두었다.
...
> **앞으로 채울 자리** — 고정 서브태스크 템플릿, 다중 턴 데이터 수집, 모델별·시나리오별 입력
> 충분성 판정, Qwen/Llama 모델 독립성 검증은 이 분리판에는 아직 기술돼 있지 않다. 별도 개정에서
> 1.6·2.2·3.1에 편입한다.
...
v1.12에서 이 자리는 **도메인 허브 구조**였다 — `McpDomain` enum 하나가 MCP 엔드포인트(`/mcp/{slug}`)·`tools/list` 필터·웹 UI 경로를 동시에 결정하고, 채팅에서는 `DomainIntentDetector`가 양쪽 도메인의 어휘 수를 세어 더 많은 쪽으로 요청을 보냈다. 도메인이 하나뿐인 v2.0에서는 비교할 상대가 없으므로 그 구조를 그대로 두면 **판정 없는 판정기**가 남는다. 따라서 이 절의 설계는 전부 덜어냈다.

이 자리에 들어올 계층은 아직 설계되지 않았다. 확정되면 다음을 이 절에 기술한다.

| 항목 | 상태 |
|---|--- |
| 요청 유형 판별(시뮬레이터 생성 요청 / 일반 장량동 질의 / 지원하지 않는 요청) | 미설계 |
| 고정 서브태스크 명세와 상태 전이 | 미설계 |
| 사용자별 세션에서의 다중 턴 데이터 수집 | 미설계 |
| 수집한 답변의 정규화와 입력 충분성 판정 | 미설계 |
| 충분해졌을 때의 시나리오 조립과 도구 연결 | 미설계 |
| 부족할 때의 재질문·실패 처리 | 미설계 |
...
> **앞으로 채울 자리** — 고정 서브태스크 불변성, Qwen/Llama 동일성, 사용자 답변 정규화, 누락 데이터
> 재요청, 입력 충분성 판정, 사용자 세션 격리, 종단간 시뮬레이터 생성 흐름 검증은 아직 이 문서에 없다.
> 해당 설계가 SDD 2.2에 들어오면 함께 편입한다.
