...
>
> **구현 현황(2026-09-01)** — 그 계층은 **코드에 들어갔고, v2.1에서 이 문서로 옮겨 담았다**
> — 요구사항 1.6.1, 설계 2.2, 테스트 3.13. 원 서술은 통합 명세
> `docs_waste-sim-spring_SRS_SDD_TDD_v1_13`의 1.14·2.18·3.17에 있고, 질문 목록은
> `jangnyang_fixed_subtasks`에 있다. 옮기면서 실제 구현과 어긋나 있던 충분성 판정은 구현
> 쪽으로 맞췄다(FR-59). v1.13 부록 A.4.2는 남은 불일치를 계속 기록한다.
...
> 설계 **2.2**(D-23~32), 테스트 **3.13**(UT-90~108·IT-31~34). 질문 목록 자체는 별도 명세
> `jangnyang_fixed_subtasks`에 있고, 이 문서는 그 목록의 개별 문장을 옮겨 담지 않는다 —
> 세트는 리소스가 소유하고 버전과 해시로 식별된다(FR-49·50).
...
위 FR-10~23은 **값이 이미 문장에 들어 있는** 요청을 다룬다. 그런데 “장량동 원룸촌 시뮬레이터를 만들어 줘” 같은 **생성 요청**은 그 경로로 답할 수 없다 — 필요한 값이 쉰 개에 이르고, 사용자는 무엇이 필요한지 모른다. 이 절은 그 값을 여러 턴에 걸쳐 모으는 계층을 정의한다.
...
| **FR-57** | **진행 상태를 반환하되, 서브태스크 ID는 사용자에게 노출하지 않는다.** 서브태스크는 사용자용 **단계**로 묶여 있고, 화면에는 `현재 단계 n/N`·단계 이름·단계 설명·그 단계 안에서의 질문 번호만 보인다. ID는 서버의 추적과 재질문 대상 지정에만 쓴다 — 쉰 개를 한 줄로 세우면 사용자가 자기 위치를 알 수 없기 때문이다. | 필수 |
...
| **NFR-16 (v2.1)** | **진행 상태의 지속성** | 진행 중인 수집은 서버 재시작을 견뎌야 한다. **현재 미충족** — 저장소가 in-memory이고, 이는 의도적으로 미룬 범위다. 저장소 인터페이스가 이 상태를 드러내고 회귀 테스트가 고정한다(3.13). |
...
사용자 요청
  ...
  ← 사용자 답변
  → 답변 정규화               OpenAiService.normalizeToField (FR-54, LLM)
...
세트의 서브태스크는 서버의 추적 단위이고, 사용자에게는 **단계**로 묶여 보인다. 화면에 나가는 것은 `현재 단계 n/N`·단계 이름·단계 설명·그 단계 안의 질문 번호이며, 서브태스크 ID는 나가지 않는다(FR-57). 두 단위를 분리하는 이유는 단순하다 — 쉰 개를 한 줄로 세우면 사용자가 자기 위치를 알 수 없고, 그렇다고 ID를 감추면 서버가 어느 항목을 다시 물어야 하는지 지정할 수 없다.
