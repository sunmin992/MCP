...
public enum DataQualityFlag {
    ...
    /**
     * 사용자가 답하지 않아 <b>시뮬레이터의 기본값으로 채운</b> 항목이 있다.
     *
     * <p>이 표시가 없으면 밖에서 대조할 수 있는 값(포항시 표준데이터, TMAP 측정)과 이
     * 시뮬레이터가 그냥 정해 둔 값이 결과에서 구별되지 않는다. 둘 다 "기본값"으로 보이지만
     * 뒷받침의 성질이 다르다.
     */
    MODEL_DEFAULT_USED(
            "다음 값은 사용자가 답하지 않아 시뮬레이터 기본값으로 채웠습니다: %s. "
                    + "밖에서 대조할 출처가 있는 값이 아니라 이 모델이 정해 둔 값입니다.");
