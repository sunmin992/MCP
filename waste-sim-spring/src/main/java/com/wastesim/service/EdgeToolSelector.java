...
public final class EdgeToolSelector {
     ...
     *
     * <p><b>최종 리뷰 회귀 — 배치 어휘가 다른 도구의 주제를 훔친다.</b> 맨 "배치"·"조합"에는
     * 부정 가드가 없어서, 문장에 다른 도구의 주제 어휘가 함께 있어도 FAN_LAYOUT이 먼저
     * 걸려 버렸다. "팬 rpm 조합을 스윕해줘"·"팬 속도와 전력 조합 중 가성비가 제일 좋은
     * 건?"은 SWEEP이 진짜 주제인데 배치 랭킹으로 샜고, "팬 달린 상태에서 방열판 배치
     * 비교해줘"·"쿨러 달았을 때 방열판 배치 비교해줘"는 {@link #HEATSINK} 주석이 이미
     * 두 번 기록한 결함(냉각 조건을 비교 대상으로 착각)의 세 번째 재현이다 — 사용자는
     * 방열판 형상을 물었는데 팬 위치 순위표가 돌아왔다. "팬 흡기 배치 데이터를 CSV로
     * 기록했는데 모델 보정할 수 있어?"도 마찬가지로 CALIBRATE의 몫이었다.
     *
     * <p>그래서 다른 도구의 주제를 가리키는 어휘(스윕/rpm류, 캘리브레이션류, 방열판
     * 명사 자체)가 함께 있으면 FAN_LAYOUT은 물러난다 — 순서를 바꾸지 않고, 대신 이
     * 문장이 원래 검사 순서를 타고 SWEEP·CALIBRATE·HEATSINK로 자연스럽게 떨어지게
     * 한다. "최적"은 가드에 넣지 않는다 — "최적 팬 배치"가 SWEEP의 "최적...팬"에게
     * 도로 뺏기기 때문이다(바로 위 문단 참고).
     */
    private static final Pattern FAN_LAYOUT = Pattern.compile(
            ...
            + "))"
            + "(?!.*(스윕|sweep|rpm|알피엠|pwm|회전수|속도|가성비|전력"
            + "|보정|캘리브|calibrat|csv|실측"
            + "|방열판|히트싱크))",
