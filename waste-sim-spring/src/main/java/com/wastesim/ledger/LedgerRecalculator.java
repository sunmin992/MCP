...
/**
 * 구조를 바꾸는 답변이 들어왔을 때 결정기록을 다시 계산한다.
 *
...
public final class LedgerRecalculator {
    ...
    public List<ParameterDecision> onAnswerChanged(ParameterLedger ledger,
        ...
        //    슈퍼시드했다"는 거짓 기록이 지우지 못하는 결정기록에 영구히 남는다. 첫 답을
     ...
     *
     * <p>{@code recordDecision}이 이미 새 결정을 결정기록에 쌓아 둔 뒤 이 메서드가 불린다 —
    ...
    private static boolean upstreamGenuinelyChanged(ParameterLedger ledger, String changedParameterId) {
        ...
        // 결정에는 "상위 값이 바뀌어 슈퍼시드했다"는 거짓이 지우지 못하는 결정기록에
     ...
     * 결정기록이 알 방법은 없다. 알 수 없는 것을 골라내려 하는 대신 전부 다시 묻는 쪽을 택한다 —
